...
export default function AdminPricing() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('/api/setup-pricing', {
        method: 'POST',
      })
      ...
      if (response.ok) {
        setStatus({ type: 'success', message: 'Page Tarifs créée avec succès dans Sanity Studio!' })
        setTimeout(() => {
  ...
  return (
    <Container>
      <Title>Administration - Page Tarifs</Title>
      
      <Card>
        <Description>
          Créez automatiquement la page "Tarifs" dans Sanity Studio avec des plans tarifaires 
          professionnels, grille de fonctionnalités et formulaire de contact intégré.
        </Description>
        ...
        >
          {isGenerating ? 'Génération en cours...' : 'Créer la Page Tarifs dans Studio'}
        </Button>
